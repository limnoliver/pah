...
An important first step is to visualize the data to see the magnitude and ranges of PAH concentrations across compounds, sites, or other grouping variables of interest. This can be accomplished using the `plot_pca` function. If your dataset includes a grouping ID (e.g., sites nested within watershets), you can order and label the data by groups. 
...
Next, we'll show a sample by source tile plot, and color code whether the source is impossible (> TOC concentration), unlikely (> 0.5\*TOC concentration), or possible (< 0.5\*TOC concentration). [The dataframe generated by this seems correct, but plotting looks broken. Need to fix.]


```{r, warnings = FALSE, messages = FALSE}
calc_mass_fractions(my_dat_c, sample_column = 'site_no', conc_column = 'result_va', compound_column = 'Parameter', plot = TRUE, calc_type = 'by_sample')
