...
#' @param compound_info table with compound metadata. A built-in table is the default but may not contain
...
get_compound_info <- function(pah_dat, merge_type = "name", merge_col = "parameter_nm", compound_info = pah_compounds){
  temp_dat <- pah_dat
  ...
  if (merge_type == "name") {
    compound_info$Parameter_stripped <- tolower(gsub(pattern = "[[:punct:]]|[[:blank:]]|alkylated|s", replacement = "", compound_info$Parameter))
    temp_dat$Parameter_stripped <- tolower(gsub(pattern = "[[:punct:]]|[[:blank:]]|alkylated|s", replacement = "", pah_dat[[merge_col]]))
    temp_dat <- left_join(temp_dat, compound_info, by = "Parameter_stripped") %>%
      select(-Parameter_stripped)

    if (any(is.na(temp_dat$EPApriority16))) {
      warning(paste0("Not all compound names in column ", merge_col, " matched a compound name in the built-in data frame compound_info, so function did not perform a complete merge. Rows with compound names that did not match are included and were given NA values for the new columns."))
    }
  }
  if (merge_type == "pcode") {
    if (is.numeric(temp_dat[[merge_col]])) {
      warning("Pcodes provided are numeric, converted to character string to match data type in compound_info")
      temp_dat[merge_col] <- as.character(temp_dat[[merge_col]])
    }
    temp_dat <- left_join(temp_dat, compound_info, by = setNames('pcode', merge_col))

    if (any(is.na(temp_dat$EPApriority16))) {
      warning(paste0("Not all codes in column ", merge_col, " matched a code in the built-in data frame compound_info, so function did not perform a complete merge. Rows with compound names that did not match are included and were given NA values for the new columns."))
    }
  }
  if (merge_type == "casrn") {

    temp_dat <- left_join(temp_dat, compound_info$pcode, by = setNames("casrn" = merge_col))

    if (any(is.na(temp_dat$EPApriority16))) {
      warning(paste0("Not all codes in column ", merge_col, " matched a code in the built-in data frame compound_info, so function did not perform a complete merge. Rows with codes that did not match are included and were given NA values for the new columns."))
    }
  }
  return(temp_dat)
