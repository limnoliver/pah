#' pah_pca
#'
#' Condenses the 12-compound PAH profile data from sources and samples into mutliple axes that explain
#' the covariance of the compounds. Then, the Euclidean distance between each source and sample combination
#' is computed.
#'
#' @export
#' @param pah_dat "long" dataframe with PAH concentrations, where there is one column of compounds, a column for
#' unique sample identification, and a column for the concentration value. If individual samples are to be
#' averaged before computation, the user should supply the average values.
...
#' dataframe all columns in `source_profiles`, as well as the chi-squared difference between source
#' and sample, and the proportional concentrations. The second dataframe is a long data frame where
#' observations are repeated for each sample/source combination, and the chi2 value is summed across
#' all compounds to create a distance metric between each sample and source profile.
#' @import dplyr
